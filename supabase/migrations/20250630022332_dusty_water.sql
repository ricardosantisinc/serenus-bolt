/*
  ...
  1. Nova tabela
    - `subscription_plans` - Armazena planos de assinatura disponíveis
      - `id` (uuid, chave primária)
      - `name` (text) - Nome do plano
      - `value` (numeric) - Valor do plano
      - `periodicity` - Periodicidade ('monthly', 'quarterly', 'annually')
      - `features` (text[]) - Array de recursos incluídos
      - `is_active` (boolean) - Indica se o plano está ativo
      - `created_at` (timestamptz)
      - `description` (text, opcional) - Descrição detalhada do plano
  
  2. Segurança
    - Enable RLS na tabela subscription_plans
    - Políticas para super_admin: acesso total
    - Políticas para gerentes: apenas leitura
*/
